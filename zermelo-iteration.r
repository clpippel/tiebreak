...
# Show relative Elo rating, Zermelo Ur.
# Die Berechnung der Turnier-Ergebnische als ein Maximumproblem der Wahrscheinlichkeitsrechnung.
# Zermelo's calculations in the above document are somewhat inaccurate.
# New York 1924 chess tournament.
z1 <- exp(rrtg / (400 / log(10)))                   # product is 1.
ur <- round(z1 / sum(z1) * 100, 2)                  # sum is 100.
d2 <- data.frame(Player = rdtable[, 2], rrtg = rrtg, Ur = ur)
d2
